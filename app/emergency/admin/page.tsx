...
async function fetchEmergencyDocuments(): Promise<EmergencyDocument[]> {
...
export default function EmergencyDocumentAdminPage() {
  ...
  const {
    ...
    mutate,
  } = useSWR<EmergencyDocument[]>(isAuthenticated ? "emergency-documents-admin" : null, fetchEmergencyDocuments)
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      void mutate()
      window.alert("비상대응 절차서가 등록되었습니다.")
      setTitle("")
      setFile(null)
  ...
  const startEditing = (document: EmergencyDocument) => {
  ...
  const handleDelete = async (document: EmergencyDocument) => {
      ...
      <main className="flex min-h-dvh items-center justify-center bg-muted px-4 py-8">
        <section className="w-full max-w-sm rounded-3xl border border-border bg-card p-6 shadow-lg" aria-labelledby="admin-login-title">
          <div className="flex flex-col gap-2 text-center">
            ...
            <p className="text-sm leading-6 text-muted-foreground">비상대응 절차서를 관리하려면 관리자 정보를 입력해 주세요.</p>
          </div>
    ...
    <main className="mx-auto min-h-dvh w-full max-w-md bg-background text-foreground shadow-sm">
      <header className="flex h-16 items-center gap-2 border-b border-border px-4">
        <button
          ...
          aria-label="비상대응 절차서 목록으로 이동"
        >
          <ArrowLeft className="size-5" aria-hidden="true" />
        </button>
        <h1 className="text-lg font-bold tracking-tight">비상대응 절차서 관리</h1>
      </header>
      ...
      <section className="flex flex-col gap-6 px-4 py-6" aria-labelledby="upload-form-title">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm leading-6 text-muted-foreground">현장에서 확인할 비상대응 절차서를 업로드하세요.</p>
        </div>

        <form onSubmit={handleSubmit} className="flex flex-col gap-5 rounded-2xl border border-border bg-card p-5 shadow-sm">
          <label className="flex flex-col gap-2" htmlFor="emergency-document-title">
            <span className="text-sm font-semibold">문서 제목</span>
            <input
              id="emergency-document-title"
              type="text"
              value={title}
          ...
          <label className="flex flex-col gap-2" htmlFor="emergency-document-file">
